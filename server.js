...
app.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    // Prompt asks for just the answer, not description
    const userQuestion = req.body.question || "What is the main message or most important object in this image?"; // Support custom question, fallback to generic
    const userPrompt = `Given the image, return only a concise answer to the following question: "${userQuestion}". Do not describe the image; just answer the question directly.`;
    ...
    try {
      ...
      db.run('INSERT INTO answers (question, answer) VALUES (?, ?)', [userQuestion, answer || 'No answer returned']);
    } catch (err) {
      return res.status(500).json({ error: 'Invalid JSON from Perplexity', details: responseText });
    }

    // Return only JSON with Q/A pair
    res.json({
